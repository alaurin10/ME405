'''!@file                   TMC4210_Class.py
    @brief                  Class for TMC4210 driver
    @details                Class to handle all communication
                            and control of the stepper motors
...
class TMC4210:
    ...
    def __init__(self, V_MIN, V_MAX, A_MAX, STEP_LENGTH, MOTOR):                    
    ...
    def set_step_duration(self, step_length):
        step_duration = round((step_length * self.CLK_FREQ / 16000000) - 1)

        en_sd = bytearray([0,0,0,0])
        en_sd[3] |= 1 << 5
    ...
    def set_pulseRamp_div(self, v_max, a_max):
        ...
        current_byte = bytearray([0,0,0,0])
    ...
    def set_current_position(self, position):
        position_val = bytearray([0,0,0,0])
    ...
    def set_max_speed(self, speed):
        if type(speed) == bytearray:
            self.write_register(V_MAX_W, speed)
        else:
            v_max_val = bytearray([0,0,0,0])
            v_max_val[3] = speed & 0xFF
            v_max_val[2] = (speed & 0x700) >> 8
            self.write_register(V_MAX_W, v_max_val)

    def set_target_speed(self, speed):
        speed_val = bytearray([0,0,0,0])
    ...
    def read_register(self, address):
        ...
        if self.MOTOR == 1:        
            self.CS1.low()
            self.spi.send_recv(address, buf)
            self.CS1.high()
        elif self.MOTOR == 2:
            self.CS2.low()
            self.spi.send_recv(address, buf)
...
IF_CONF_W =    bytearray([0b01101000,0,0,0b00100000])      
